...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            selectInput(
                'e0',
                '0. An ordinary select input',
                choices = state.name,
                selectize = FALSE
            ),
            selectizeInput(
                'e1',
                '1. A basic example (zero-configuration)',
                choices = state.name
            ),
            selectizeInput(
                'e2',
            ...
            selectizeInput(
                'e5',
                '5. Max number of items to select',
                choices = state.name,
                multiple = TRUE,
            ...
            selectInput(
                'e7',
                '7. selectInput() does not respond to empty strings',
                choices = state.name
            )
        ),
        mainPanel(
      ...
      $.ajax({
        ...
        success: function(res) {
          callback(res.repositories.slice(0, 10));
            ...
            helpText(
                'If the above searching fails, it is probably the Github API limit
...
server <- function(input, output) {
    ...
    output$github <- renderText({
        paste(
            'You selected',
            if (input$github == '') 'nothing' else input$github,
